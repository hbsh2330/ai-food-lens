...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _saveProfile() async {
    ...
    if (height == null ||
      ...
      setState(() => _message = '키와 몸무게를 숫자로 입력해 주세요.');
      return;
    }
    await FoodDataApi.saveProfile(UserProfile(
        heightCm: height, weightKg: weight, targetWeightKg: target));
    if (mounted) setState(() => _message = '내 정보를 저장했어요.');
  }
  ...
  Future<void> _recordThisMonth() async {
    ...
    final record = WeightRecord(date: now, weightKg: weight);
            ...
            : ListView(
                ...
                children: [
                  ...
                  if (_message != null) ...[
                    const SizedBox(height: 12),
                    _MessageCard(
                        message: _message!, isError: _message!.contains('입력')),
                  ],
                  const SizedBox(height: 22),
